		...
		<div id="menu" style="position: absolute; top: 0; left: 0; z-index: 10; width: 200px; min-height: 410px; background: #F8F8F8; display: inline-block;"><!-- border-radius: 3px; border: 1px solid #00C5Cb; -->
			...
			<div style="position: relative; top: 10px; left: 10px; width: 180px;">
				<div id="item_0" style="font-size: 12px; font-family: Verdana, serif; text-transform: lowercase; line-height: 1.7em;">
					<a href="<?php echo '/'.$array_directory[1].'/worker/process/statistics.php?typeMenu=0'; ?>" class="getInterval"><span style="color: #004345;">Пользователи</span></a></div>
				<div id="item_2" style="font-size: 12px; font-family: Verdana, serif; text-transform: lowercase; line-height: 1.7em;">
					<a href="<?php echo '/'.$array_directory[1].'/worker/process/statistics.php?typeMenu=1'; ?>" class="getInterval"><span style="color: #004345;">Геопозиционирование</span></a></div>
				<div id="item_3" style="font-size: 12px; font-family: Verdana, serif; text-transform: lowercase; line-height: 1.7em;">
					<a href="<?php echo '/'.$array_directory[1].'/worker/process/statistics.php?typeMenu=2'; ?>" class="getInterval"><span style="color: #004345;">Типы компьютерных атак</span></a></div>
				<div id="item_4" style="font-size: 12px; font-family: Verdana, serif; text-transform: lowercase; line-height: 1.7em;">
					<a href="<?php echo '/'.$array_directory[1].'/worker/process/statistics.php?typeMenu=3'; ?>" class="getInterval"><span style="color: #004345;">Кол-во компьютерных атак</span></a></div>
				<div id="item_5" style="font-size: 12px; font-family: Verdana, serif; text-transform: lowercase; line-height: 1.7em;">
					<a href="<?php echo '/'.$array_directory[1].'/worker/process/statistics.php?typeMenu=4'; ?>" class="getInterval"><span style="color: #004345;">Соотношение компьютерных атак</span></a></div>
			</div>
		</div>
...
<!-- поле диаграмм -->
					...
					$ShowStatisticsForUsers->showInformation(ShowStatisticsForUsers::WORKER);
					 ?>
					</div>
					<?php
				break;
...
<!-- вывод статистической информации по 'умолчанию' -->
					...
					function getInfo(elem){
						...
						switch(elem.firstChild.name){
							...
							case 'tab_1':
								var newObjectXMLHttpRequest = new objectXMLHttpRequest('POST', "/<?= $array_directory[1] ?>/worker/process/ajax_process.php", elem, 'queryStatistics=GeoIp_1');
								newObjectXMLHttpRequest.sendRequest();
							break;
							//информация по странам для IP-адресов назначения
							case 'tab_2': 
								var newObjectXMLHttpRequest = new objectXMLHttpRequest('POST', "/<?= $array_directory[1] ?>/worker/process/ajax_process.php", elem, 'queryStatistics=GeoIp_2');
								newObjectXMLHttpRequest.sendRequest();							
							break;
							//информация за отрезок времени по странам и IP-адресам назначения
							case 'tab_3':
								var newObjectXMLHttpRequest = new objectXMLHttpRequest('POST', "/<?= $array_directory[1] ?>/worker/process/ajax_process.php", elem, 'queryStatistics=GeoIp_3');
								newObjectXMLHttpRequest.sendRequest();
							break;
							//информация по ложным компьютерным атакам
							case 'tab_4':
								var newObjectXMLHttpRequest = new objectXMLHttpRequest('POST', "/<?= $array_directory[1] ?>/worker/process/ajax_process.php", elem, 'queryStatistics=GeoIp_4');
								newObjectXMLHttpRequest.sendRequest();
							break;
							}
						}
