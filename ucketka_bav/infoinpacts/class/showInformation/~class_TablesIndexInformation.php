...
static public function showTableWaitAnalysis($GeoIP, $userId) 
	...
	$SeverityRatingData = new SeverityRatingData;
	try{
 		$query = self::linkDataBase()->connectionDB()->query("SELECT t1.id, `date_time_incident_start`, `date_time_incident_end`, `date_time_create`, `ip_src`, `ip_dst`, `space_safe`
 														  	  FROM `incident_chief_tables` t0 INNER JOIN `incident_additional_tables` t1 
 														  	  ON t0.id = t1.id LEFT JOIN `incident_analyst_tables` t2 ON t1.id = t2.id 
 														  	  WHERE ((t2.id) is Null) GROUP BY `ip_src`, t1.id ORDER BY `date_time_incident_start`");
		//временный массив под IP-адреса источников		
		$array_tmp = $array_ip = array();
		while($row = $array_tmp[] = $query->fetch(PDO::FETCH_ASSOC)){
			$array_ip[$row['id']][] = $row['ip_src'];
