...
public function showBlockShortInformation()
	{
	?>
	<style type="text/css">
	.propertiesText {
...
<!-- воздействий за год -->
		<div id="allInformationYear" class="propertiesText" style="text-align: center;">
			<span style="font-size: 12px;">всего: </span>
			<span style="font-size: 20px;"><?= self::getCountImpactYear($this->getCurrentYear()) ?></span> 
		</div>
<!-- проанализированных воздействий за год -->
        <div id="allAnalyseInformationYear" class="propertiesText" style="text-align: center;">
            <span style="font-size: 12px;">проанализированных: </span>
            <span style="font-size: 20px;"><?= self::allAnalyseInformationYear($this->getCurrentYear()) ?></span>
        </div>
<!-- не рассмотренно воздействий за год -->
        <div id="allCountTraffikNotLook" class="propertiesText" style="text-align: center;">
            <span style="font-size: 12px;">не рассмотренных: </span>
            <span style="font-size: 20px;"><?= self::getCountTraffikNotLook($this->getCurrentYear()) ?></span>
        </div>
<!-- ложных воздействий за год -->
		<div id="falseInformationYear" class="propertiesText" style="text-align: center;">
			<span style="font-size: 12px;">ложных: </span>
...
//функция вывода общего количества проанализированных компьютерных воздействий за определенный год
public static function allAnalyseInformationYear($year)
    {
        $checkedYear = self::checkYear($year);
        try{
            $query = self::getConnectionDB()->connectionDB()->query("SELECT COUNT(`id`) AS COUNT_ID FROM
                                                                    (SELECT t0.id FROM `incident_chief_tables` t0 INNER JOIN `incident_analyst_tables` t1
                                                                    ON t0.id = t1.id WHERE YEAR(`date_time_incident_start`)='".$checkedYear."' AND (t1.true_false='1'
                                                                    OR t1.true_false='2') GROUP BY `id`) AS t0");
            return $query->fetch(PDO::FETCH_OBJ)->COUNT_ID;
        }
        catch(PDOException $e){
            echo MessageErrors::userMessageError(MessageErrors::ERROR_SQL,"\t файл: ".$e->getFile()."\t линия: ".$e->getLine()."\t ошибка: ".$e->getMessage());
...
//функция вывода общего количества проанализированных компьютерных воздействий за определенный год
public static function getCountAnalyseInformationYear($year)
    {
        $checkedYear = self::checkYear($year);
        try{
            $query = self::getConnectionDB()->connectionDB()->query("SELECT COUNT(`id`) AS COUNT_ID FROM
...
//функция вывода информации об общем количестве непроанализированных компьютерных воздействиях
public static function getCountTraffikNotLook($year)
    {
        $checkedYear = self::checkYear($year);
        try{
            $query = self::getConnectionDB()->connectionDB()->query("SELECT COUNT(`id`) AS COUNT_ID FROM (SELECT t0.id FROM `incident_chief_tables` t0 JOIN `incident_analyst_tables` t1
																 ON t0.id=t1.id WHERE YEAR(`date_time_incident_start`)='".$checkedYear."' AND `true_false`='5' GROUP BY `id`) AS t0");
            return $query->fetch(PDO::FETCH_OBJ)->COUNT_ID;
        }
        catch(PDOException $e){
            echo MessageErrors::userMessageError(MessageErrors::ERROR_SQL,"\t файл: ".$e->getFile()."\t линия: ".$e->getLine()."\t ошибка: ".$e->getMessage());
...
// 											     				 ON t0.id = t1.id WHERE t1.true_false='5'");
    }
...
//функция вывода информации о не выполненном анализе компьютерного воздействия
private function showCountNotSeeAnalyst()
	...
	try{
		$query = self::getConnectionDB()->connectionDB()->query("SELECT t0.id FROM `incident_chief_tables` t0 INNER JOIN `incident_additional_tables` t1 ON t0.id = t1.id 
																 LEFT JOIN `incident_analyst_tables` t2 ON t1.id = t2.id WHERE t2.id is Null GROUP BY t1.id");
		while($row = $query->fetch(PDO::FETCH_OBJ)){
			$array[] =$row->id;
			}
		return $array;
...
private function showCountNotMail()
	...
	try{
		$query = self::getConnectionDB()->connectionDB()->query("SELECT t0.id FROM `incident_additional_tables` t0 LEFT JOIN `incident_analyst_tables` t1 
 											     				 ON t0.id = t1.id WHERE (t1.true_false='1' OR t1.true_false='5') AND (t0.number_mail_in_CIB is Null)");
		while($row = $query->fetch(PDO::FETCH_OBJ)){
			$array[] =$row->id;
			}
		return $array;
...
// - сетевой трафик не рассматривался
