...
public function giveLinks($var_name, $start) 
	...
	//	общему количеству найденных элементов ($all_num) и если $string_limit = 0 
	...
	if($start > 1){
		...
		$linksOut .= "<span class='page_active'>&laquo;</span>
					  <span class='page_active'>&lsaquo;</span>";	
		}
	...
	foreach($this->array_pages[$nead_chunk] as $pageNum => $ofset)
		...
		if($ofset == $start){
			$linksOut .= "<span class='page_active'>".$pageNum."</span>";
			continue;	
			}
	...
	if(($this->allNum - $this->stringLimit) > $start){
		...
		$linksOut .= "<span class='page_active'>&rsaquo;</span>
					  <span class='page_active'>&raquo;</span>";
		}
	return $linksOut;
	}
