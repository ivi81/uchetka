<?php
						...
						/*----------------------------------------------------------*/
						/*  	класс редактирования таблиц БД (администратор)		*/
...
/*
id - порядковый номер инцидента
availability_host - доступность ресурса
...
space_safe - место расположение отфильтрованного сетевого трафика
explanation - пояснение дежурного
solution - решение дежурного (изменяется автоматически)
*/
	//проверяем переданные параметры
	$this->checkKeysArray(array('id', 'ping', 'direction', 'number_mail_in_CIB', 'number_mail_in_organization', 'explanation', 'spaceSafeTraff'));	
	//решение дежурного по компьютерному воздействию (отправка писем)
	$solution = '';
	try{
		//проверяем является поле number_mail_in_CIB пустым, если да то не записываем информацию об этом поле в таблицу
		if($this->array_info_impact['number_mail_in_CIB'] == ''){
			//если пустое
			$query = $this->DBO->connectionDB()->prepare("UPDATE `incident_additional_tables` 
												 		  SET `availability_host`='".$this->array_info_impact['ping']."',
												 		 `direction_attack`='".$this->array_info_impact['direction']."',
												 		 `space_safe`=:spaceSafeTraff, 
												 		 `explanation`=:explanation,
												 		 `solution`='".$solution."'
												 		  WHERE `id`='".$this->array_info_impact['id']."'");

			$query->execute(array(':spaceSafeTraff' => $this->array_info_impact['spaceSafeTraff'],
								  ':explanation' => $this->array_info_impact['explanation']));			
			} else {
			$solution = "подготовлено письмо в 18 Центр ФСБ России";
			if($this->array_info_impact['number_mail_in_organization'] != ''){
				$solution = "подготовлено письмо в 18 Центр ФСБ России а также в сопутствующую организацию";
				}
			//если не пустое
			$query = $this->DBO->connectionDB()->prepare("UPDATE `incident_additional_tables` 
												 		  SET `availability_host`='".$this->array_info_impact['ping']."',
			...
			$query->execute(array(':number_mail_in_CIB' => $this->array_info_impact['number_mail_in_CIB'],
								  ':number_mail_in_organization' => $this->array_info_impact['number_mail_in_organization'],
								  ':spaceSafeTraff' => $this->array_info_impact['spaceSafeTraff'],
								  ':explanation' => $this->array_info_impact['explanation']));
			}
		}
	catch(PDOException $e){
...
/*
id - порядковый номер инцидента
true_false - решение аналитика
count_alert_analyst - количество срабатываний
information_analyst - информация
*/
	//проверяем переданные параметры
	$this->checkKeysArray(array('id', 'solution', 'analystCount','analystInfo'));
	try{
		if($this->array_info_impact['solution'] == 0){
			$this->deleteIncidentAnalistTables();
			} else {
			$query = $this->DBO->connectionDB()->prepare("UPDATE `incident_analyst_tables` 
														  SET `true_false`=:solution,
														  `information_analyst`=:analistInfo,
														  `count_alert_analyst`=:analystCount
														  WHERE `id`='".$this->array_info_impact['id']."'");
			$query->execute(array(':solution' => $this->array_info_impact['solution'],
								  ':analystCount' => $this->array_info_impact['analystCount'],
								  ':analistInfo' => $this->array_info_impact['analystInfo']));
			}
		}
	catch(PDOException $e){
		echo MessageErrors::userMessageError(MessageErrors::ERROR_SQL,"\t файл: ".$e->getFile()."\t линия: ".$e->getLine()."\t ошибка: ".$e->getMessage());
...
//удаляем записи из таблицы  incident_analyst_tables
public function deleteIncidentAnalistTables()
	{
	$this->DBO->connectionDB()->query("DELETE FROM `incident_analyst_tables` WHERE `id`='".$this->array_info_impact['id']."'");
	}
