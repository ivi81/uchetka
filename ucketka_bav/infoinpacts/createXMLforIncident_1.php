...
/*
 действие аналитика по отношению к анализируемому компьютерному воздействию
  R - компьютерная атака
  C - ложное срабатывание
  N - в данный момент нет сетевого трафика
...
*/
	...
	private $arrayTegXml = array( 
		"Info" => array( 
			...
			"AvailabilityHost" => "availability_host",
			"DirectionAttack" => "direction_attack",
	...
	public function createFile(){
		...
		try{
			$query = self::getLinkDB()->query("SELECT t1.id, {$stringColumns}
 											   ...
 											   ON t0.id = t1.id {$numComputerIncident} ORDER BY t1.id ASC");
...
echo "<b>IncidentId - ".$arrayTmp[$a]['id'].'</b><br>';
//Information
					  ...
					  $info = $incidentId->appendChild($xmlDomDocument->createElement('Info'));
					   ...
					   $info->appendChild($xmlDomDocument->createElement('Explanation', htmlspecialchars($explanation)));
					   $info->appendChild($xmlDomDocument->createElement('AvailabilityHost', $arrayTmp[$a]['availability_host']));
					   $info->appendChild($xmlDomDocument->createElement('DirectionAttack', $arrayTmp[$a]['direction_attack']));
					   ...
					   $info->appendChild($xmlDomDocument->createElement('UserAnalyst', GetUserName::userName($arrayInformationAnalyst['UserAnalyst'])));
					   $timeAnalyst = (empty($arrayInformationAnalyst['TimeAnalyst'])) ? '-': $arrayInformationAnalyst['TimeAnalyst']; 
					   $info->appendChild($xmlDomDocument->createElement('TimeAnalyst', $arrayInformationAnalyst['TimeAnalyst']));
					   $info->appendChild($xmlDomDocument->createElement('InformationAnalyst', htmlspecialchars($arrayInformationAnalyst['InformationAnalyst'])));
					   $info->appendChild($xmlDomDocument->createElement('DstIp', $arrayTmp[$a]['ip_dst']));
					   $info->appendChild($xmlDomDocument->createElement('DstName', htmlspecialchars(GetDomainName::domainName(long2ip($arrayTmp[$a]['ip_dst'])))));
//Sources
...
//Signatures
					...
					foreach ($arrayInformationSignatures as $signatureId => $signatureMessage) {
					   $signatures->appendChild($xmlDomDocument->createElement('SignatureId', htmlspecialchars($signatureId)));
					   $signatures->appendChild($xmlDomDocument->createElement('SignatureMessage', htmlspecialchars($signatureMessage)));
					}
				}			
				$i = $arrayTmp[$a]['id'];
			}
...
//$createXmlFile = new CreateXmlFile(array('3696', '3706', '3755'));
