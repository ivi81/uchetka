...
function addSrcIP()
	...
	if(string.indexOf('addComputerImpact') != -1){
		var nameSrcIp = document.getElementById('srcIp');
		//клонируем форму с информацией по сигнатурам
		var cloneSrcIp = nameSrcIp.cloneNode(true);
		//создаем дополнительную форму
		var newNode = nameSrcIp.parentNode.insertBefore(cloneSrcIp, document.getElementById('exitId'));
		//очищаем атрибуты
		newNode.firstChild.nextSibling.firstChild.nextSibling.setAttribute('value', '');
		//удаляем ненужный элемент DIV с информацией о ранее встречавшихся IP-адресах источниках
		...
		//выбираем второго ребенка главного элемента div (первый ребенок является текстовой нодой)
		var divChild = srcIp.firstChild.nextSibling.childNodes;
		//создаем объект клонированных элементов 
		clonElements = {};
		for(var i = 0; i < divChild.length; i++){
			...
			if(divChild[i].nodeType == 1 && divChild[i].getAttribute('name') == 'ipSrc[]'){
				var cloneSrcIp = divChild[i].cloneNode(true); 
				//очищаем атрибуты					
			...
			if(divChild[i].nodeType == 1 && divChild[i].getAttribute('name') == 'ipNum[]'){
				var cloneSrcIpNum = divChild[i].cloneNode(true);
				//очищаем атрибуты
...
function addSid()
	...
	//клонируем форму с информацией по сигнатурам
	var cloneSid = nameSid.cloneNode(true);
	//создаем дополнительную форму
	var newNode = nameSid.parentNode.insertBefore(cloneSid, document.getElementById('stopId'));
	//очищаем форму с номером сигнатуры и ее количеством
...
function delSid()
	{
	var nameSid = document.getElementById('nameSid');
	var trNode	= nameSid.parentNode.childNodes;
	//получаем количество элементов с номером сигнатуры
