...
function checkDate(directory) {
  ...
  function setNewYearImg() {
    ...
    createImgElement({
      id: 'snowman',
      src: '/' + directory + '/img/holiday/snowman_1.png',
      style: 'position: absolute; top: 30px; left: 20px;'
    }); //ёлка

    createImgElement({
      id: 'tree',
      src: '/' + directory + '/img/holiday/tree_1.png',
      style: 'position: absolute; top: 25px; left: 850px;'
    }); //надпись 'с новым годом'

    createImgElement({
      id: 'stringNewYear',
      src: '/' + directory + '/img/holiday/new_year_2.png',
    ...
    var pathname = window.location.pathname; //если главная страница

    if (pathname.indexOf('index') !== -1 && pathname.indexOf('major') === -1) {
      //подарок
      createImgElement({
        id: 'gift',
        src: '/' + directory + '/img/holiday/gift_1.png',
        style: 'position: absolute; top: 340px; left: 895px; z-index: 70;'
      }); //ёлочный шар

      createImgElement({
        id: 'tree_ballon_4',
        src: '/' + directory + '/img/holiday/tree_ballon_4.png',
        style: 'position: absolute; top: 215px; left: 10px; z-index: 70;'
      });
    ...
    if (pathname.indexOf('edit_incidents') > 0) {
      ...
      createImgElement({
        id: 'tree_ballon_1',
        src: '/' + directory + '/img/holiday/xmas.png',
        style: 'position: absolute; top: 265px; left: 883px; z-index: 70;'
      });
    ...
    if (pathname.indexOf('search_incidents') > 0 && pathname.indexOf('major') === -1) {
      ...
      createImgElement({
        id: 'snegir',
        src: '/' + directory + '/img/holiday/snegir.png',
        style: 'position: absolute; top: 235px; left: 40px; z-index: 70;'
      }); //ёлочная игрушка снеговик

      createImgElement({
        id: 'snowmanIcon',
        src: '/' + directory + '/img/holiday/Snowman-icon.png',
        style: 'position: absolute; top: 435px; left: 931px; z-index: 70;'
      });
    ...
    if (pathname.indexOf('analysis_incident') > 0) {
      ...
      createImgElement({
        id: 'tree_ballon_3',
        src: '/' + directory + '/img/holiday/tree_ballon_3.png',
        style: 'position: absolute; top: 669px; left: 785px; z-index: 70;'
      });
    }
  ...
  function createImgElement(objElements) {
    if (_typeof(objElements) !== 'object') return false; //родительский элемент
...
function checkDateAuthorization(directory) {
  ...
  if (objDate.getMonth() == 11 && objDate.getDate() > 24 || objDate.getMonth() == 0 && objDate.getDate() < 15) {
    var div = document.getElementById('author'); //добавляем сниговика
    ...
    div.appendChild(divNew_1); //добавляем сосульки
