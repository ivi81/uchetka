...
function objectXMLHttpRequest(method, path, elem, arg) {
  //метод запроса
  this.requestMethod = method; //путь к файлу обработчику

  this.requestPathFile = path; //аргументы (для GET и POST запросов)

  this.requestArg = arg; //идентификатор элемента (для вывода информации)

  this.element = elem; //обработчик ответа
    ...
    request.onreadystatechange = function () {
      switch (argTest[0]) {
        ...
        case 'querySid':
          ...
          var infoElement = elem.parentNode.parentNode.firstChild.nextSibling.nextSibling.nextSibling; //проверяем завершена ли передача

          if (request.readyState == 4) {
            var div = infoElement.appendChild(document.createElement('DIV'));
            div.setAttribute('style', 'text-align: center;'); //проверяем статус ответа сервера
          ...
          } else {//вывод индикатора загрузки
          }
          ...
          if (request.readyState == 4) {
            ...
            if (request.status == 200) {
              if (typeof request.responseText == 'string') {
                var array = request.responseText.split(':'); //выбранные пользователем год

                divChoiceYear.innerHTML = '<span style="font-size: 12px;">воздействия за </span><span style="font-size: 20px; color: #3300FF;">' + array[0] + '</span><span style="font-size: 12px;"> год</span>'; //общая информация за выбранный год

                divAllInformationYear.innerHTML = '<span style="font-size: 12px;">всего: </span><span style="font-size: 20px;">' + array[1] + '</span>'; //общая информация за выбранный год

                divAllAnalyseInformationYear.innerHTML = '<span style="font-size: 12px;">проанализированных: </span><span style="font-size: 20px;">' + array[2] + '</span>'; //общая информация за выбранный год

                divAllNotAnalyseInformationYear.innerHTML = '<span style="font-size: 12px;">не рассмотренных: </span><span style="font-size: 20px;">' + array[3] + '</span>'; //ложные компьютерные воздействия за выбранный год
              ...
              switch (argGeoIp) {
                ...
                case 'GeoIp_2':
                  ...
                  document.onload = function () {
                    ...
                    var areaIntervalDate = document.getElementById('areaIntervalDate'); //отправка запроса

                    var areaSendRequest = document.getElementById('areaSendRequest'); //вывод диаграммы
                ...
                case 'GeoIp_4':
                  ...
                  document.onload = function () {
                    //отправка запроса
                    var areaSendRequest = document.getElementById('areaSendRequest'); //вывод интервала времени
...
function showLoadingDetector(elem, request) {
  ...
  var infoElement = elem.parentNode.parentNode.firstChild.nextSibling.nextSibling.nextSibling; //создаем новый элемент DIV

  var div = infoElement.appendChild(document.createElement('DIV'));
  div.setAttribute('style', 'text-align: center;'); //получаем имя директории сайта

  var nameSite = window.location.href.split('/'); //создаем элемент с изображением
...
objectXMLHttpRequest.prototype.createRequest = function () {
  ...
  */
  var request = false; //для стандартных браузеров
...
objectXMLHttpRequest.prototype.sendRequest = function () {
  ...
  */
  ...
  request.open(this.requestMethod, this.requestPathFile, true); //если POST запрос

  if (this.requestMethod.toUpperCase() == 'POST' && this.requestMethod.length > 0) {
    //устанавливаем заголовок
    request.setRequestHeader("Content-Type", "application/x-www-form-urlencoded; charset=utf-8", "Cache-Control", "no-store, no-cache, must-revalidate, max-age=0");
