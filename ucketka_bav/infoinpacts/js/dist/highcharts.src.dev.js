...
(function () {
  ...
  seriesTypes = {},
      Highcharts; // The Highcharts namespace
    ...
    var i,
      ...
      for (key in original) {
        if (original.hasOwnProperty(key)) {
          ...
          if (value && _typeof(value) === 'object' && Object.prototype.toString.call(value) !== '[object Array]' && key !== 'renderTo' && typeof value.nodeType !== 'number') {
  ...
  function attr(elem, prop, value) {
    var key, ret; // if the prop is a string
  ...
  function numberFormat(number, decimals, decPoint, thousandsSep) {
    ...
    return externalFn !== numberFormat ? externalFn(number, decimals, decPoint, thousandsSep) : s + (j ? i.substr(0, j) + t : "") + i.substr(j).replace(/(\d{3})(?=\d)/g, "$1" + t) + (c ? d + mathAbs(n - i).toFixed(c).slice(2) : "");
  }
  /**
  ...
  function normalizeTickInterval(interval, multiples, magnitude, allowDecimals) {
    var normalized, i; // round to a tenfold of 1, 2, 2.5 or 5

    magnitude = pick(magnitude, 1);
    normalized = interval / magnitude; // multiples for a linear scale

    if (!multiples) {
      multiples = [1, 2, 2.5, 5, 10]; // the allowDecimals option
  ...
  error = function error(code, stop) {
  ...
  pathAnim = {
     ...
     */
    init: function init(elem, fromD, toD) {
      fromD = fromD || '';
     ...
     */
    step: function step(start, end, pos, complete) {
      var ret = [],
          i = start.length,
          startVal;
    ...
    win.HighchartsAdapter = win.HighchartsAdapter || $ && {
      ...
      init: function init(pathAnim) {
        ...
        $.each(['cur', '_default', 'width', 'height', 'opacity'], function (i, fn) {
          var obj = Fx.step,
              base; // Handle different parent objects
        ...
        this.addAnimSetter('d', function (fx) {
          var elem = fx.elem,
              ends; // Normally start and end should be set in state == 0, but sometimes,
        ...
        $.fn.highcharts = function () {
          ...
          if (this[0]) {
            ...
            options = args[0]; // Create the chart
       ...
       */
      addAnimSetter: function addAnimSetter(prop, setter) {
        // jQuery 1.8 style
        if ($.Tween) {
       ...
       */
      adapterRun: function adapterRun(elem, method) {
        return $(elem)[method]();
      },
       ...
       */
      removeEvent: function removeEvent(el, eventType, handler) {
       ...
       */
      fireEvent: function fireEvent(el, type, eventArguments, defaultFunction) {
        var event = $.Event(type),
            detachedType = 'detached' + type,
            defaultPrevented; // Remove warnings in Chrome when accessing returnValue (#2790), layerX and layerY. Although Highcharts
        ...
        extend(event, eventArguments); // Prevent jQuery from triggering the object method that is named the
       ...
       */
      washMouseEvent: function washMouseEvent(e) {
        var ret = e.originalEvent || e; // computed by jQuery, needed by IE8
       ...
       */
      animate: function animate(el, params, options) {
        var $el = $(el);
  ...
  })(win.jQuery); // check for a custom HighchartsAdapter defined prior to this file


  var globalAdapter = win.HighchartsAdapter,
      adapter = globalAdapter || {}; // Initialize the adapter
  ...
  var defaultPlotOptions = defaultOptions.plotOptions,
      defaultSeriesOptions = defaultPlotOptions.line; // set the default time methods
  ...
  SVGElement.prototype = {
     ...
     */
    colorGradient: function colorGradient(color, prop, elem) {
      var renderer = this.renderer,
          ...
          n,
          id,
          key = []; // Apply linear or radial gradients
      ...
      if (gradName) {
        ...
        key = key.join(','); // Check if a gradient object with the same config object is created within this renderer
        ...
        if (gradients[key]) {
          ...
          gradients[key] = gradientObject = renderer.createElement(gradName).attr(gradAttr).add(renderer.defs); // The gradient needs to keep a list of stops to be able to destroy them
    ...
    attr: function attr(hash, val) {
      var key,
          ...
          hasSetSymbolSize,
          ret = this,
          skipAttr; // single key-value pair
    ...
    updateShadows: function updateShadows(key, value) {
    ...
    */
    ...
    css: function css(styles) {
      var elemWrapper = this,
          ...
          serializedCss = '',
          hyphenate,
          hasNew = !oldStyles; // convert legacy
      ...
      if (hasNew) {
        textWidth = elemWrapper.textWidth = styles && styles.width && elem.nodeName.toLowerCase() === 'text' && pInt(styles.width); // Merge the new styles with the old ones
        ...
        if (isIE && !hasSVG) {
          _css(elemWrapper.element, styles);
        } else {
          /*jslint unparam: true*/
    ...
    updateTransform: function updateTransform() {
      var wrapper = this,
          ...
          transform; // flipping affects translate as adjustment for flipping around the group's axis
    ...
    align: function align(alignOptions, alignByTranslate, box) {
      var align,
          ...
          alignedObjects = renderer.alignedObjects; // First call on instanciate
      ...
      attribs[alignByTranslate ? 'translateX' : 'x'] = mathRound(x); // Vertical align
      ...
      attribs[alignByTranslate ? 'translateY' : 'y'] = mathRound(y); // Animate only if already placed
    ...
    getBBox: function getBBox() {
      var wrapper = this,
          ...
          cacheKey; // Since numbers are monospaced, and numerical labels appear a lot in a chart,
      ...
      if (!bBox) {
        ...
        if (element.namespaceURI === SVG_NS || renderer.forExport) {
          ...
          } catch (e) {} // If the bBox is not set, the try-catch block above failed. The other condition
        ...
        if (renderer.isSVG) {
          width = bBox.width;
          height = bBox.height; // Workaround for wrong bounding box in IE9 and IE10 (#1101, #1505, #1669, #2568)
    ...
    add: function add(parent) {
      ...
      this.added = true; // fire an event for internal hooks
    ...
    dashstyleSetter: function dashstyleSetter(value) {
      ...
      if (value) {
        ...
        value = value.join(',').replace('NaN', 'none'); // #3226
    ...
    zIndexSetter: function zIndexSetter(value, key, element) {
      element.setAttribute(key, value);
      this[key] = value;
    },
  ...
  SVGElement.prototype['stroke-widthSetter'] = SVGElement.prototype.strokeSetter = function (value, key, element) {
    this[key] = value; // Only apply the stroke attribute if the stroke width is defined and larger than 0
  ...
  SVGRenderer.prototype = {
     ...
     */
    init: function init(container, width, height, style, forExport) {
      var renderer = this,
          loc = location,
          boxWrapper,
          element,
      ...
      container.appendChild(element); // For browsers other than IE, add the namespace attribute (#1978)
      ...
      renderer.alignedObjects = []; // Page url used for internal references. #24, #672, #1070
      ...
      if (isFirefox && container.getBoundingClientRect) {
        renderer.subPixelFix = subPixelFix = function subPixelFix() {
          _css(container, {
            left: 0,
            top: 0
          });
        ...
        each(lines, function (line, lineNo) {
          ...
          each(spans, function (span) {
            if (span !== '' || spans.length === 1) {
              ...
              span = (span.replace(/<(.|\n)*?>/g, '') || ' ').replace(/&lt;/g, '<').replace(/&gt;/g, '>'); // Nested tags aren't supported, and cause crash in Safari (#1596)
              ...
              if (span !== ' ') {
                ...
                if (width) {
                  ...
                  while (hasWhiteSpace && (words.length || rest.length)) {
                    ...
                    actualWidth = bBox.width; // Old IE cannot measure the actualWidth for SVG elements (#2314)
    ...
    button: function button(text, x, y, callback, normalState, hoverState, pressedState, disabledState, shape) {
      ...
      delete normalState.style; // Hover state
      ...
      delete hoverState.style; // Pressed state
      ...
      delete pressedState.style; // Disabled state
      ...
      delete disabledState.style; // Add the events. IE9 and IE10 need mouseover and mouseout to funciton (#667).
     ...
     * @param {Number} height
     */
    image: function image(src, x, y, width, height) {
      var attribs = {
        preserveAspectRatio: NONE
      },
          elemWrapper; // optional properties
      ...
      elemWrapper = this.createElement('image').attr(attribs); // set the href in the xlink namespace
    ...
    symbol: function symbol(_symbol, x, y, width, height, options) {
      ...
      if (path) {
      ...
      } else if (imageRegex.test(_symbol)) {
        // On image load, set the size and position
        centerImage = function centerImage(img, size) {
          if (img.element) {
        ...
        imageSize = symbolSizes[imageSrc] || options && options.width && options.height && [options.width, options.height]; // Ireate the image synchronously, add attribs async
    ...
    text: function text(str, x, y, useHTML) {
      ...
      wrapper = renderer.createElement('text').attr(attr); // Prevent wrapping from creating false offsetWidths in export in legacy IE (#1079, #1063)
      ...
      if (!useHTML) {
        wrapper.xSetter = function (value, key, element) {
          ...
          for (i = 0; i < tspans.length; i++) {
            tspan = tspans[i]; // If the x values are equal, the tspan represents a linebreak
    ...
    label: function label(str, x, y, shape, anchorX, anchorY, useHTML, baseline, className) {
      ...
      function updateTextPadding() {
        var styles = wrapper.styles,
            ...
            y; // determin y based on the baseline
      ...
      wrapper.onAdd = function () {
        ...
        wrapper.attr({
          text: str || str === 0 ? str : '',
          // alignment is available now // #3295: 0 not rendered if given as a value
          x: x,
          y: y
        });
  ...
  extend(SVGElement.prototype, {
     ...
     * by the VML renderer
     */
    htmlCss: function htmlCss(styles) {
      var wrapper = this,
          element = wrapper.element,
    ...
    htmlGetBBox: function htmlGetBBox() {
      var wrapper = this,
          element = wrapper.element,
          bBox = wrapper.bBox; // faking getBBox in exported SVG in legacy IE
    ...
    htmlUpdateTransform: function htmlUpdateTransform() {
      ...
      var wrapper = this,
        ...
        right: 1
          ...
          shadows = wrapper.shadows; // apply translate
      ...
      if (elem.tagName === 'SPAN') {
        ...
        if (currentTextTransform !== wrapper.cTT) {
          ...
          width = pick(wrapper.elemWidth, elem.offsetWidth); // Update textWidth
     ...
     */
    setSpanRotation: function setSpanRotation(rotation, alignCorrection, baseline) {
      var rotationStyle = {},
          cssTransformKey = isIE ? '-ms-transform' : isWebKit ? '-webkit-transform' : isFirefox ? 'MozTransform' : isOpera ? '-o-transform' : '';
      rotationStyle[cssTransformKey] = rotationStyle.transform = 'rotate(' + rotation + 'deg)';
      rotationStyle[cssTransformKey + (isFirefox ? 'Origin' : '-origin')] = rotationStyle.transformOrigin = alignCorrection * 100 + '% ' + baseline + 'px';

      _css(this.element, rotationStyle);
    },
  ...
  extend(SVGRenderer.prototype, {
     ...
     * @param {Number} y
     */
    html: function html(str, x, y) {
      var wrapper = this.createElement('span'),
          element = wrapper.element,
          renderer = wrapper.renderer; // Text setter
          ...
          if (svgGroupWrapper) {
            ...
            if (!htmlGroup) {
              ...
              each(parents.reverse(), function (parentGroup) {
                var htmlGroupStyle; // Create a HTML div and append it to the parent div to emulate
                ...
                extend(parentGroup, {
                  translateXSetter: function translateXSetter(value, key) {
                    htmlGroupStyle.left = value + PX;
                    parentGroup[key] = value;
                    parentGroup.doTransform = true;
                  },
                  translateYSetter: function translateYSetter(value, key) {
                    htmlGroupStyle.top = value + PX;
                    parentGroup[key] = value;
                    parentGroup.doTransform = true;
                  },
                  visibilitySetter: function visibilitySetter(value, key) {
  ...
  if (!hasSVG && !useCanVG) {
    ...
    VMLElement = {
      ...
      init: function init(renderer, nodeName) {
        var wrapper = this,
            ...
            isDiv = nodeName === DIV; // divs and shapes need size
        ...
        markup.push(' style="', style.join(''), '"/>'); // create element with default attributes and style
       ...
       */
      getSpanCorrection: function getSpanCorrection(width, baseline, alignCorrection, rotation, align) {
        var costheta = rotation ? mathCos(rotation * deg2rad) : 1,
            sintheta = rotation ? mathSin(rotation * deg2rad) : 0,
            height = pick(this.elemHeight, this.element.offsetHeight),
            quad,
            nonLeft = align && align !== 'left'; // correct x and y

        this.xCorr = costheta < 0 && -width;
        this.yCorr = sintheta < 0 && -height; // correct for baseline and corners spilling out after rotation

        quad = costheta * sintheta < 0;
        this.xCorr += sintheta * baseline * (quad ? 1 - alignCorrection : alignCorrection);
        this.yCorr -= costheta * baseline * (rotation ? quad ? alignCorrection : 1 - alignCorrection : 1); // correct for the length/height of the text
       ...
       */
      pathToVML: function pathToVML(value) {
        // convert paths
        var i = value.length,
            path = [];
        ...
        while (i--) {
          ...
          if (isNumber(value[i])) {
            ...
            path[i] = value[i]; // When the start X and end X coordinates of an arc are too close,
       ...
       */
      cutOffPath: function cutOffPath(path, length) {
        var len;
        path = path.split(/[ ,]/);
        len = path.length;
      ...
      shadow: function shadow(shadowOptions, group, cutOff) {
        var shadows = [],
            ...
            modifiedPath,
            shadowWidth,
            shadowElementOpacity; // some times empty paths are not strings
        ...
        if (shadowOptions) {
          ...
          for (i = 1; i <= 3; i++) {
            strokeWidth = shadowWidth * 2 + 1 - 2 * i; // Cut off shadows for stacked column items
            ...
            createElement(renderer.prepVML(markup), null, null, shadow); // insert it
      ...
      classSetter: function classSetter(value) {
        // IE8 Standards mode has problems retrieving the className unless set like this
        this.element.className = value;
      },
      dashstyleSetter: function dashstyleSetter(value, key, element) {
      ...
      rotationSetter: function rotationSetter(value, key, element) {
      ...
      strokeSetter: function strokeSetter(value, key, element) {
        this.setAttr('strokecolor', this.renderer.color(value, element, key));
      },
      'stroke-widthSetter': function strokeWidthSetter(value, key, element) {
      ...
      titleSetter: function titleSetter(value, key) {
        this.setAttr(key, value);
      },
      visibilitySetter: function visibilitySetter(value, key, element) {
        ...
        if (element.nodeName === 'DIV') {
          value = value === HIDDEN ? '-999em' : 0; // In order to redraw, IE7 needs the div to be visible when tucked away
      ...
      xSetter: function xSetter(value, key, element) {
    ...
    Highcharts.VMLElement = VMLElement = extendClass(SVGElement, VMLElement); // Some shared setters
    ...
    var VMLRendererExtension = {
       ...
       * @param {Number} height
       */
      init: function init(container, width, height, style) {
        var renderer = this,
            boxWrapper,
            box,
            css;
        ...
        container.appendChild(boxWrapper.element); // generate the containing box

        renderer.isVML = true;
        renderer.box = box;
        renderer.boxWrapper = boxWrapper;
        renderer.cache = {};
        renderer.setSize(width, height, false); // The only way to make IE6 and IE7 print is to use a global namespace. However,
        // with IE8 the only way to make the dynamic shapes visible in screen and print mode
        // seems to be to add the xmlns attribute and the behaviour style inline.

        if (!doc.namespaces.hcv) {
          doc.namespaces.add('hcv', 'urn:schemas-microsoft-com:vml'); // Setup default CSS (#2153, #2368, #2384)
      ...
      clipRect: function clipRect(x, y, width, height) {
        ...
        var clipRect = this.createElement(),
            isObj = isObject(x); // mimic a rectangle with its style object for automatic updating in attr
        ...
        return extend(clipRect, {
          ...
          getCSS: function getCSS(wrapper) {
       ...
       */
      color: function color(_color, elem, prop, wrapper) {
        var renderer = this,
            colorObject,
            regexRgba = /^rgba/,
            markup,
            fillType,
            ret = NONE; // Check for linear or radial gradient

        if (_color && _color.linearGradient) {
          fillType = 'gradient';
        } else if (_color && _color.radialGradient) {
          fillType = 'pattern';
        }
        ...
        if (fillType) {
          var stopColor,
              stopOpacity,
              gradient = _color.linearGradient || _color.radialGradient,
              x1,
              y1,
              x2,
              y2,
              ...
              lastStop,
              colors = [],
              addFillNode = function addFillNode() {
          ...
          if (prop === 'fill') {
            ...
            if (fillType === 'gradient') {
              ...
              var r = gradient.r,
                  ...
                  bBox,
                  applyRadialGradient = function applyRadialGradient() {
                if (radialReference) {
                  bBox = wrapper.getBBox();
        ...
        } else if (regexRgba.test(_color) && elem.tagName !== 'IMG') {
          colorObject = Color(_color);
       ...
       */
      prepVML: function prepVML(markup) {
       ...
       */
      g: function g(name) {
        var wrapper, attribs; // set the class name
       ...
       */
      invertChild: function invertChild(element, parentNode) {
        var ren = this,
      ...
      symbols: {
        ...
        circle: function circle(x, y, w, h, wrapper) {
          if (wrapper) {
            w = h = 2 * wrapper.r;
         ...
         */
        rect: function rect(x, y, w, h, options) {
          return SVGRenderer.prototype.symbols[!defined(options) || !options.r ? 'square' : 'callout'].call(0, x, y, w, h, options);
        }
      }
    };

    Highcharts.VMLRenderer = VMLRenderer = function VMLRenderer() {
      this.init.apply(this, arguments);
    };

    VMLRenderer.prototype = merge(SVGRenderer.prototype, VMLRendererExtension); // general renderer
  ...
  if (useCanVG) {
     ...
     */
    Highcharts.CanVGRenderer = CanVGRenderer = function CanVGRenderer() {
    ...
    CanVGController = function () {
      ...
      function drawDeferred() {
        var callLength = deferredRenderCalls.length,
            callIndex; // Draw all pending render calls
      ...
      return {
        push: function push(func, scriptLocation) {
  ...
  Tick.prototype = {
    ...
    addLabel: function addLabel() {
      var tick = this,
          ...
          width = horiz && categories && !labelOptions.step && !labelOptions.staggerLines && !labelOptions.rotation && chart.plotWidth / tickPositions.length || !horiz && (chart.margin[3] || chart.chartWidth * 0.33),
          // #1580, #1931
      isFirst = pos === tickPositions[0],
          ...
          dateTimeLabelFormat; // Set the datetime label format. If a higher rank is set for this position, use that. If not,
      ...
      tick.isLast = isLast; // get the string
      ...
      if (!defined(label)) {
        ...
        tick.label = label = defined(str) && labelOptions.enabled ? chart.renderer.text(str, 0, 0, labelOptions.useHTML).attr(attr) // without position absolute, IE export sometimes is wrong
        .css(extend(css, labelOptions.style)).add(axis.labelGroup) : null; // Set the tick baseline and correct for rotation (#1764)
     ...
     */
    handleOverflow: function handleOverflow(index, xy) {
      var show = true,
          axis = this.axis,
          ...
          justifyToPlot; // Hide it if it now overlaps the neighbour label
      ...
      if (justifyLabel) {
        ...
        axisRight = justifyToPlot ? axisLeft + axis.len : axis.chart.chartWidth; // Find the firsth neighbour on the same line
        ...
        if (isFirst && !reversed || isLast && reversed) {
          ...
          if (pxPos + leftSide < axisLeft) {
            ...
            pxPos = axisLeft - leftSide; // Hide it if it now overlaps the neighbour label
          ...
          if (pxPos + rightSide > axisRight) {
            ...
            pxPos = axisRight - rightSide; // Hide it if it now overlaps the neighbour label
     ...
     */
    getPosition: function getPosition(horiz, pos, tickmarkOffset, old) {
      var axis = this.axis,
          chart = axis.chart,
    ...
    getLabelPosition: function getLabelPosition(x, y, label, horiz, labelOptions, tickmarkOffset, index, step) {
      ...
      y = y + this.yOffset - (tickmarkOffset && !horiz ? tickmarkOffset * transA * (reversed ? 1 : -1) : 0); // Correct for staggered labels
    ...
    render: function render(index, old, opacity) {
      ...
      if (label && !isNaN(x)) {
        label.xy = xy = tick.getLabelPosition(x, y, label, horiz, labelOptions, tickmarkOffset, index, step); // Apply show first and show last. If the tick is both first and last, it is
  ...
  Highcharts.PlotLineOrBand.prototype = {
    ...
    render: function render() {
      var plotLine = this,
          ...
          attribs = {},
          renderer = axis.chart.renderer; // logarithmic conversion
      ...
      if (svgElem) {
        ...
        plotLine.svgElem = svgElem = renderer.path(path).attr(attribs).add(); // events

        if (events) {
          addEvent = function addEvent(eventType) {
      ...
      if (optionsLabel && defined(optionsLabel.text) && path && path.length && axis.width > 0 && axis.height > 0) {
        ...
        }, optionsLabel); // add the SVG element
  ...
  AxisPlotLineOrBandExtension = {
     ...
     */
    getPlotBandPath: function getPlotBandPath(from, to) {
    ...
    addPlotBand: function addPlotBand(options) {
      return this.addPlotBandOrLine(options, 'plotBands');
    },
    addPlotLine: function addPlotLine(options) {
     ...
     */
    addPlotBandOrLine: function addPlotBandOrLine(options, coll) {
     ...
     * @param {Object} id
     */
    removePlotBandOrLine: function removePlotBandOrLine(id) {
  ...
  Axis.prototype = {
    ...
    init: function init(chart, userOptions) {
      ...
      axis.isDatetimeAxis = isDatetimeAxis; // Flag, if axis is linked to another axis
      ...
      axis.tickmarkOffset = axis.categories && options.tickmarkPlacement === 'between' && pick(options.tickInterval, 1) === 1 ? 0.5 : 0; // #3202
      // Major ticks

      axis.ticks = {};
      axis.labelEdge = []; // Minor ticks

      axis.minorTicks = {}; //axis.tickAmount = UNDEFINED;
      ...
      axis.stacks = {};
      axis.oldStacks = {}; // Min and max in the data
      ...
      var eventType,
          events = axis.options.events; // Register
      ...
      axis.removePlotLine = axis.removePlotBandOrLine; // register event listeners
    ...
    setOptions: function setOptions(userOptions) {
      this.options = merge(this.defaultOptions, this.isXAxis ? {} : this.defaultYAxisOptions, [this.defaultTopAxisOptions, this.defaultRightAxisOptions, this.defaultBottomAxisOptions, this.defaultLeftAxisOptions][this.side], merge(defaultOptions[this.coll], // if set in setOptions (#1053)
      userOptions));
    },
      ...
      each(axis.series, function (series) {
        if (series.visible || !chart.options.chart.ignoreHiddenSeries) {
          ...
          if (axis.isXAxis) {
            ...
            seriesDataMin = series.dataMin; // Get the dataMin and dataMax so far. If percentage is used, the min and max are
    ...
    adjustForMinRange: function adjustForMinRange() {
      var axis = this,
          ...
          loopLength,
          minArgs,
          maxArgs; // Set the automatic minimum range based on the closest point distance
    ...
    setAxisTranslation: function setAxisTranslation(saveOld) {
      var axis = this,
          ...
          transA = axis.transA; // Adjust translation for padding. Y axis with categories need to go through the same (#1784).
      ...
      if (axis.isXAxis || hasCategories || pointRange) {
        if (linkedParent) {
          ...
          each(axis.series, function (series) {
            ...
            pointRange = mathMax(pointRange, seriesPointRange); // minPointOffset is the value padding to the left of the axis in order to make
        ...
        axis.pointRangePadding = pointRangePadding = pointRangePadding * ordinalCorrection; // pointRange means the width reserved for each point, like in a column chart
     ...
     * to the nearest tick
     */
    setTickPositions: function setTickPositions(secondPass) {
      var axis = this,
          chart = axis.chart,
          options = axis.options,
          ...
          tickPositions,
          keepTwoTicksOnly,
          categories = axis.categories; // linked axis gets the extremes from the parent axis
      ...
      if (axis.min === axis.max || axis.min === undefined || axis.max === undefined) {
        ...
        (axis.max - axis.min) * tickPixelIntervalOption / mathMax(axis.len, tickPixelIntervalOption)); // For squished axes, set only two ticks
      ...
      if (!isDatetimeAxis && !isLog) {
        ...
        if (!tickIntervalOption) {
          axis.tickInterval = normalizeTickInterval(axis.tickInterval, null, getMagnitude(axis.tickInterval), // If the tick interval is between 1 and 5 and the axis max is in the order of
      ...
      axis.minorTickInterval = options.minorTickInterval === 'auto' && axis.tickInterval ? axis.tickInterval / 5 : options.minorTickInterval; // find the tick positions
      ...
      if (!tickPositions) {
        ...
        if (isDatetimeAxis) {
          tickPositions = axis.getTimeTicks(axis.normalizeTimeTickInterval(axis.tickInterval, options.units), axis.min, axis.max, options.startOfWeek, axis.ordinalPositions, axis.closestPointRange, true);
        } else if (isLog) {
    ...
    adjustTickAmount: function adjustTickAmount() {
      ...
      if (maxTicks && maxTicks[key] && !axis.isDatetimeAxis && !axis.categories && !axis.isLinked && axis.options.alignTicks !== false && this.min !== UNDEFINED) {
        var oldTickAmount = axis.tickAmount,
            calculatedTickAmount = tickPositions.length,
            tickAmount; // set the axis-level tickAmount to use below
    ...
    setScale: function setScale() {
      ...
      if (isDirtyAxisLength || isDirtyData || axis.isLinked || axis.forceRedraw || axis.userMin !== axis.oldUserMin || axis.userMax !== axis.oldUserMax) {
        ...
        axis.setTickPositions(); // record old values to decide whether a rescale is necessary later on (#540)

        axis.oldUserMin = axis.userMin;
        axis.oldUserMax = axis.userMax; // Mark as dirty if it is not already set to dirty and extremes have changed. #595.
    ...
    setExtremes: function setExtremes(newMin, newMax, redraw, animation, eventArguments) {
      ...
      fireEvent(axis, 'setExtremes', eventArguments, function () {
        ...
        axis.userMin = newMin;
        axis.userMax = newMax;
        axis.eventArgs = eventArguments; // Mark for running afterSetExtremes

        axis.isDirtyExtremes = true; // redraw
    ...
    zoom: function zoom(newMin, newMax) {
      var dataMin = this.dataMin,
          ...
          options = this.options; // Prevent pinch zooming out of range. Check for defined is for #1946. #1734.
    ...
    setAxisSize: function setAxisSize() {
      var chart = this.chart,
          ...
          percentRegex = /%$/; // Check for percentage based input values
    ...
    getOffset: function getOffset() {
      ...
      if (hasData || axis.isLinked) {
        ...
        axis.labelAlign = pick(labelOptions.align || axis.autoLabelAlign(labelOptions.rotation)); // Generate ticks
    ...
    render: function render() {
      ...
      axis.justifyToPlot = overflow === 'justify'; // Mark all elements inActive before we go over and mark the active ones
  ...
  Axis.prototype.getTimeTicks = function (normalizedInterval, min, max, startOfWeek) {
    ...
    if (defined(min)) {
      ...
      var time = minDate.getTime(),
          ...
          localTimezoneOffset = (timeUnits.day + (useUTC ? timezoneOffset : minDate.getTimezoneOffset() * 60 * 1000)) % timeUnits.day; // #950, #3359
      // iterate and add tick positions at appropriate values

      while (time < max) {
        tickPositions.push(time); // if the interval is years, use Date.UTC to increase years
  ...
  */
  ...
  Tooltip.prototype = {
    init: function init(chart, options) {
      var borderWidth = options.borderWidth,
          style = options.style,
          padding = pInt(style.padding); // Save the chart and options

      this.chart = chart;
      this.options = options; // Keep track of the current series
    ...
    move: function move(x, y, anchorX, anchorY) {
      var tooltip = this,
          now = tooltip.now,
          animate = tooltip.options.animation !== false && !tooltip.isHidden && ( // When we get close to the target position, abort animation and land on the right place (#3056)
    ...
    hide: function hide(delay) {
      ...
      if (!this.isHidden) {
        ...
        }, pick(delay, this.options.hideDelay, 500)); // hide previous hoverPoints and set new
    ...
    getAnchor: function getAnchor(points, mouseEvent) {
      var ret,
          ...
          plotY = 0,
          yAxis;
      points = splat(points); // Pie uses a special tooltipPos
    ...
    getPosition: function getPosition(boxWidth, boxHeight, point) {
       ...
       */
      firstDimension = function firstDimension(dim, outerSize, innerSize, point) {
    ...
    defaultFormatter: function defaultFormatter(tooltip) {
      ...
      s = [tooltip.tooltipHeaderFormatter(items[0])]; // build the values
     ...
     * @param {Object} point
     */
    refresh: function refresh(point, mouseEvent) {
      var tooltip = this,
          chart = tooltip.chart,
          label = tooltip.label,
          ...
          currentSeries;
      clearTimeout(this.hideTimer); // get the reference point coordinates (pie charts use tooltipPos)
    ...
    updatePosition: function updatePosition(point) {
      var chart = this.chart,
          label = this.label,
          pos = (this.options.positioner || this.getPosition).call(this, label.width, label.height, point); // do the move
     ...
     */
    tooltipHeaderFormatter: function tooltipHeaderFormatter(point) {
      var series = point.series,
          ...
          n; // Guess the best date format based on the closest point distance (#568)

      if (isDateTime && !xDateFormat) {
        if (closestPointRange) {
          for (n in timeUnits) {
            if (timeUnits[n] >= closestPointRange || // If the point is placed every day at 23:59, we need to show
  ...
  var hoverChartIndex; // Global flag for touch support
  ...
  Pointer.prototype = {
    ...
    init: function init(chart, options) {
      var chartOptions = options.chart,
          ...
          inverted = chart.inverted,
          zoomX,
          zoomY; // Store references

      this.options = options;
      this.chart = chart; // Zoom status

      this.zoomX = zoomX = /x/.test(zoomType);
      this.zoomY = zoomY = /y/.test(zoomType);
      this.zoomHor = zoomX && !inverted || zoomY && inverted;
      this.zoomVert = zoomY && !inverted || zoomX && inverted;
      this.hasZoom = zoomX || zoomY; // Do we need to handle click on a touch device?
    ...
    normalize: function normalize(e, chartPosition) {
      var chartX, chartY, ePos; // common IE normalizing
     ...
     */
    runPointActions: function runPointActions(e) {
      var pointer = this,
          chart = pointer.chart,
          series = chart.series,
          ...
          anchor; // shared tooltip

      if (tooltip && pointer.options.tooltip.shared && !(hoverSeries && hoverSeries.noSharedTooltip)) {
        points = []; // loop over all series and find the ones with points closest to the mouse

        i = series.length;

        for (j = 0; j < i; j++) {
          if (series[j].visible && series[j].options.enableMouseTracking !== false && !series[j].noSharedTooltip && series[j].singularTooltips !== true && series[j].tooltipPoints.length) {
      ...
      if (hoverSeries && hoverSeries.tracker && !followPointer) {
        // #2584, #2830
        // get the point
        point = hoverSeries.tooltipPoints[index]; // a new point is hovered, refresh the tooltip
    ...
    reset: function reset(allowMove, delay) {
      var pointer = this,
          ...
          hoverPoint = chart.hoverPoint,
          tooltip = chart.tooltip,
          tooltipPoints = tooltip && tooltip.shared ? chart.hoverPoints : hoverPoint; // Narrow in allowMove
    ...
    drag: function drag(e) {
      var chart = this.chart,
          ...
          panKey = chartOptions.panKey && e[chartOptions.panKey + 'Key']; // If the mouse is outside the plot area, adjust to cooordinates
    ...
    drop: function drop(e) {
      ...
      if (this.selectionMarker) {
        ...
        this.selectionMarker = this.selectionMarker.destroy(); // Reset scaling preview
    ...
    onContainerMouseDown: function onContainerMouseDown(e) {
      e = this.normalize(e); // issue #295, dragging not always working in Firefox
    ...
    onTrackerMouseOut: function onTrackerMouseOut(e) {
      ...
      if (series && !series.options.stickyTracking && !this.inClass(relatedTarget, PREFIX + 'tooltip') && relatedSeries !== series) {
        series.onMouseOut();
      }
    },
  ...
  extend(Highcharts.Pointer.prototype, {
     ...
     */
    pinchTranslate: function pinchTranslate(pinchDown, touches, transform, selectionMarker, clip, lastValidTouch) {
     ...
     */
    pinchTranslateDirection: function pinchTranslateDirection(horiz, pinchDown, touches, transform, selectionMarker, clip, lastValidTouch, forcedScale) {
      var chart = this.chart,
          xy = horiz ? 'x' : 'y',
          XY = horiz ? 'X' : 'Y',
          sChartXY = 'chart' + XY,
    ...
    pinch: function pinch(e) {
      var self = this,
          ...
          clip = {}; // On touch devices, only proceed to trigger click if a handler is defined
      ...
      if (e.type === 'touchstart') {
        ...
        each(chart.axes, function (axis) {
          if (axis.zoomEnabled) {
            var bounds = chart.bounds[axis.horiz ? 'h' : 'v'],
                ...
                absMax = mathMax(min, max); // Store the bounds for use in the touchmove handler
        ...
        self.hasPinched = hasZoom; // Scale and translate the groups to provide visual feedback during pinching
    ...
    onContainerTouchStart: function onContainerTouchStart(e) {
      var chart = this.chart;
      hoverChartIndex = chart.index;
    ...
    onContainerTouchMove: function onContainerTouchMove(e) {
  ...
  if (win.PointerEvent || win.MSPointerEvent) {
    ...
    var touches = {},
        ...
        getWebkitTouches = function getWebkitTouches() {
      var key,
          fake = [];
      ...
      return fake;
        ...
        translateMSPointer = function translateMSPointer(e, method, wktype, callback) {
      var p;
      e = e.originalEvent || e;
    ...
    extend(Pointer.prototype, {
      onContainerPointerDown: function onContainerPointerDown(e) {
      ...
      onContainerPointerMove: function onContainerPointerMove(e) {
      ...
      onDocumentPointerUp: function onDocumentPointerUp(e) {
       ...
       */
      batchMSEvents: function batchMSEvents(fn) {
  ...
  Legend.prototype = {
    ...
    renderItem: function renderItem(item) {
      ...
      if (!li) {
        ...
        item.legendItem = li = renderer.text(options.labelFormat ? format(options.labelFormat, item) : options.labelFormatter.call(item), ltr ? symbolWidth + symbolPadding : -symbolPadding, legend.baseline || 0, useHTML).css(merge(item.visible ? itemStyle : itemHiddenStyle)) // merge to prevent modifying original (#1021)
        .attr({
          align: ltr ? 'left' : 'right',
          zIndex: 2
        }).add(item.legendGroup); // Get the baseline for the first item - the font size is equal for all
      ...
      legend.itemHeight = itemHeight = mathRound(item.legendItemHeight || bBox.height); // if the item exceeds the width, start a new line
    ...
    getAllItems: function getAllItems() {
      ...
      each(this.chart.series, function (series) {
        var seriesOptions = series.options; // Handle showInLegend. If the series is linked to another series, defaults to false.
    ...
    render: function render() {
      ...
      legend.display = display = !!allItems.length; // render the items
      ...
      legend.legendHeight = legendHeight; // Now that the legend width and height are established, put the items in the 
      // final position
    ...
    handleOverflow: function handleOverflow(legendHeight) {
      var legend = this,
          ...
          pages = this.pages,
          lastY,
          allItems = this.allItems; // Adjust the height
      ...
      if (legendHeight > spaceHeight && !options.useHTML) {
        ...
        this.fullHeight = legendHeight; // Fill pages with Y positions so that the top of each a legend item defines
    ...
    scroll: function scroll(scrollBy, animation) {
      var pages = this.pages,
          ...
          pager = this.pager,
          padding = this.padding,
          scrollOffset; // When resizing while looking at the last page
  ...
  Chart.prototype = {
    ...
    init: function init(userOptions, callback) {
      ...
      var chartEvents = optionsChart.events; //this.runChartClick = chartEvents && !!chartEvents.click;
      ...
      this.isResizing = 0;
      this.options = options; //chartTitleOptions = UNDEFINED;
      ...
      var chart = this,
          eventType; // Add the chart to the global lookup

      chart.index = charts.length;
      charts.push(chart);
      chartCount++; // Set up auto resize
      ...
      chart.xAxis = [];
      chart.yAxis = []; // Expose methods and variables
    ...
    getAxes: function getAxes() {
      var chart = this,
          ...
          optionsArray,
          axis; // make sure the options are arrays and add some members
    ...
    layOutTitles: function layOutTitles(redraw) {
      ...
      if (!this.isDirtyBox && requiresDirtyBox) {
        this.isDirtyBox = requiresDirtyBox; // Redraw if necessary (#2719, #2744)		
    ...
    getChartSize: function getChartSize() {
      var chart = this,
          ...
          renderTo = chart.renderToClone || chart.renderTo; // get inner width and height from jQuery (#824)
     ...
     */
    cloneRenderTo: function cloneRenderTo(revert) {
      var clone = this.renderToClone,
          container = this.container; // Destroy the clone and bring the container back to the real renderTo div
    ...
    getContainer: function getContainer() {
      ...
      }, optionsChart.style), chart.renderToClone || renderTo); // cache the cursor (#1650)
    ...
    getMargins: function getMargins() {
      ...
      chart.resetMargins();
      axisOffset = chart.axisOffset; // Adjust for title and subtitle
    ...
    setSize: function setSize(width, height, animation) {
      var chart = this,
          chartWidth,
          chartHeight,
          fireEndResize; // Handle the isResizing counter

      chart.isResizing += 1;

      fireEndResize = function fireEndResize() {
        if (chart) {
      ...
      (globalAnimation ? _animate : _css)(chart.container, {
        width: chartWidth + PX,
        height: chartHeight + PX
      }, globalAnimation);
      chart.setChartSize(true);
      chart.renderer.setSize(chartWidth, chartHeight, animation); // handle axes
      ...
      fireEvent(chart, 'resize'); // fire endResize and set isResizing back
    ...
    drawChartBox: function drawChartBox() {
      var chart = this,
          ...
          clipBox = chart.clipBox; // Chart area
     ...
     */
    showCredits: function showCredits(credits) {
    ...
    isReadyToRender: function isReadyToRender() {
      var chart = this; // Note: in spite of JSLint's complaints, win == win.top is required
    ...
    firstRender: function firstRender() {
      var chart = this,
          options = chart.options,
          callback = chart.callback; // Check whether the chart is ready to render
      ...
      chart.linkSeries(); // Run an event after axes and series are initialized, but before render. At this stage,
    ...
    */
    splashArray: function splashArray(target, options) {
  ...
  Point.prototype = {
    ...
    init: function init(series, options, x) {
      ...
      if (series.options.colorByPoint) {
        ...
        point.color = point.color || colors[series.colorCounter++]; // loop back to zero
    ...
    applyOptions: function applyOptions(options, x) {
      ...
      point.options = point.options ? extend(point.options, options) : options; // For higher dimension series types. For instance, for ranges, point.y is mapped to point.low.
    ...
    optionsToObject: function optionsToObject(options) {
      ...
      if (typeof options === 'number' || options === null) {
        ...
        ret = options; // This is the fastest way to detect if there are individual point dataLabels that need
    ...
    tooltipFormatter: function tooltipFormatter(pointFormat) {
      ...
      var series = this.series,
          ...
          valueSuffix = seriesTooltipOptions.valueSuffix || ''; // Loop over the point array map and replace unformatted values with sprintf formatting markup
  ...
  */
  ...
  Series.prototype = {
    ...
    init: function init(chart, options) {
      ...
      series._i = chartSeries.length - 1; // Sort series according to index option (#248, #1123, #2456)
    ...
    updateParallelArrays: function updateParallelArrays(point, i) {
      var series = point.series,
          args = arguments,
          fn = typeof i === 'number' ? // Insert the value in the given position
      function (key) {
    ...
    getSymbol: function getSymbol() {
      ...
      this.getCyclic('symbol', seriesMarkerOption.symbol, this.chart.options.symbols); // don't substract radius in image symbols (#604)
    ...
    setData: function setData(data, redraw, animation, updatePoints) {
      ...
      redraw = pick(redraw, true); // If the point count is the same as is was, just run Point.update which is
      ...
      if (updatePoints !== false && dataLength && oldDataLength === dataLength && !series.cropped && !series.hasGroupedData) {
        ...
        series.data = [];
        series.options.data = data; //series.zData = zData;
        // destroy old points
    ...
    processData: function processData(force) {
      ...
      options = series.options,
          ...
          xExtremes,
          min,
          max; // If the series data or axes haven't changed, don't go through this. Return false to pass
     ...
     */
    cropData: function cropData(xData, yData, min, max) {
    ...
    getExtremes: function getExtremes(yData) {
      ...
      for (i = 0; i < yDataLength; i++) {
        x = xData[i];
        y = yData[i]; // For points within the visible range, including the first point outside the
    ...
    translate: function translate() {
      ...
      var series = this,
          ...
          threshold = options.threshold; // Translate each point
      ...
      for (i = 0; i < dataLength; i++) {
        var point = points[i],
            ...
            pointStack,
            stackValues; // Discard disallowed y values for log axes
        ...
        yAxis.translate(yValue, 0, 1, 0, 1) : UNDEFINED; // Set client related positions for mouse tracking
    ...
    animate: function animate(init) {
      var series = this,
          ...
          inverted = chart.inverted,
          sharedClipKey; // Animation option is set to true

      if (animation && !isObject(animation)) {
        animation = defaultPlotOptions[series.type].animation;
      }

      sharedClipKey = ['_sharedClip', animation.duration, animation.easing, clipBox.height].join(','); // Initialize the animation. Set up the clipping rectangle.
      ...
      if (init) {
        ...
        series.sharedClipKey = sharedClipKey; // Run the animation
      } else {
        clipRect = chart[sharedClipKey];
    ...
    afterAnimate: function afterAnimate() {
      ...
      fireEvent(this, 'afterAnimate'); // Remove the shared clipping rectancgle when all series are shown
    ...
    drawPoints: function drawPoints() {
      var series = this,
          ...
          globallyEnabled = pick(seriesMarkerOptions.enabled, !series.requireSorting || series.activePointCount < 0.5 * series.xAxis.len / seriesMarkerOptions.radius);
     ...
     */
    convertAttribs: function convertAttribs(options, base1, base2, base3) {
    ...
    getAttribs: function getAttribs() {
      ...
      i,
          ...
          attr,
          key; // series type specific modifications
        ...
        while (i--) {
          ...
          if (hasPointSpecificOptions) {
            ...
            if (!seriesOptions.marker) {
              ...
              pointStateOptionsHover.color = pointStateOptionsHover.color || !point.options.color && stateOptionsHover.color || Color(point.color).brighten(pointStateOptionsHover.brightness || stateOptionsHover.brightness).get();
            ...
            pointAttr[NORMAL_STATE] = series.convertAttribs(extend(attr, normalOptions), seriesPointAttr[NORMAL_STATE]); // inherit from point normal and series hover
    ...
    destroy: function destroy() {
      ...
      series.points = null; // Clear the animation timeout if we are destroying the series during initial animation
      ...
      erase(chart.series, series); // clear all members
     ...
     */
    getSegmentPath: function getSegmentPath(segment) {
      var series = this,
          segmentPath = [],
          step = series.options.step; // build the segment line
    ...
    getGraphPath: function getGraphPath() {
      ...
      each(series.segments, function (segment) {
        segmentPath = series.getSegmentPath(segment); // add the segment to the graph, or a single point for tracking
    ...
    invertGroups: function invertGroups() {
      var series = this,
          chart = series.chart; // Pie, go away (#1736)
    ...
    render: function render() {
      ...
      animDuration = animation && !!series.animate && chart.renderer.isSVG && pick(animation.duration, 500) || 0,
          ...
          chartSeriesGroup = chart.seriesGroup; // the group
  ...
  StackItem.prototype = {
    ...
    render: function render(group) {
      ...
      if (this.label) {
        ...
        this.label = this.axis.chart.renderer.text(str, null, null, options.useHTML) // dummy positions, actual position updated with setOffset method in columnseries
     ...
     */
    setOffset: function setOffset(xOffset, xWidth) {
      var stackItem = this,
          axis = stackItem.axis,
          chart = axis.chart,
          inverted = chart.inverted,
  ...
  Series.prototype.setStackedPoints = function () {
    ...
    var series = this,
        ...
        i,
        x,
        y; // loop over the non-null y values and read them into a local array

    for (i = 0; i < yDataLength; i++) {
      x = xData[i];
      y = yData[i];
      pointKey = series.index + ',' + i; // Read stacked values into a stack based on the x value,
      ...
      stack = stacks[key][x];
      stack.points[pointKey] = [stack.cum || 0]; // Add value to the stack total
  ...
  extend(Chart.prototype, {
     ...
     */
    addSeries: function addSeries(options, redraw, animation) {
      var series,
          chart = this;
    ...
    /**
        ...
        */
    addAxis: function addAxis(options, isX, redraw, animation) {
    ...
    showLoading: function showLoading(str) {
      var chart = this,
          ...
          setLoadingSize = function setLoadingSize() {
        if (loadingDiv) {
          _css(loadingDiv, {
  ...
  extend(Point.prototype, {
     ...
     *
     */
    update: function update(options, redraw, animation, runEvent) {
      var point = this,
          series = point.series,
          graphic = point.graphic,
          i,
      ...
      function update() {
        ...
        seriesOptions.data[i] = point.options; // redraw
     ...
     *    configuration
     */
    remove: function remove(redraw, animation) {
      var point = this,
          series = point.series,
          points = series.points,
          chart = series.chart,
          ...
          data = series.data;
      setAnimation(animation, chart);
      redraw = pick(redraw, true); // fire the event with a default handler of removing the point
  ...
  extend(Series.prototype, {
     ...
     *    configuration
     */
    addPoint: function addPoint(options, redraw, shift, animation) {
      var series = this,
          seriesOptions = series.options,
          data = series.data,
          graph = series.graph,
          ...
          x,
          i;
      setAnimation(animation, chart); // Make graph animate sideways
    ...
    remove: function remove(redraw, animation) {
      ...
      if (!series.isRemoving) {
        ...
        series.isRemoving = true; // fire the event with a default handler of removing the point
     ...
     */
    update: function update(newOptions, redraw) {
      var series = this,
          chart = this.chart,
      ...
      oldOptions = this.userOptions,
          ...
          n; // Make sure groups are not destroyed (#3094)
      ...
      }, newOptions); // Destroy the series and reinsert methods from the type prototype
      ...
      extend(this, seriesTypes[newOptions.type || oldType].prototype); // Re-register groups (#3094)
  ...
  extend(Axis.prototype, {
     ...
     */
    setTitle: function setTitle(newTitleOptions, redraw) {
      this.update({
     ...
     * @param {Boolean} redraw
     */
    setCategories: function setCategories(categories, redraw) {
      this.update({
  ...
  var AreaSeries = extendClass(Series, {
     ...
     * charts.
     */
    closeSegment: function closeSegment(path, segment, translatedThreshold) {
  ...
  var SplineSeries = extendClass(Series, {
     ...
     */
    getPointSpline: function getPointSpline(segment, point, i) {
      var smoothing = 1.5,
      ...
      if (lastPoint && nextPoint) {
        ...
        rightContY = (smoothing * plotY + nextY) / denom; // have the two control points make a straight line through main point
  ...
  var ColumnSeries = extendClass(Series, {
    ...
    getColumnMetrics: function getColumnMetrics() {
      var series = this,
          ...
          stackGroups = {},
          columnIndex,
          columnCount = 0; // Get the total number of column type series.
    ...
    translate: function translate() {
      var series = this,
          chart = series.chart,
          options = series.options,
          borderWidth = series.borderWidth = pick(options.borderWidth, series.activePointCount > 0.5 * series.xAxis.len ? 0 : 1),
          yAxis = series.yAxis,
      ...
      each(series.points, function (point) {
        ...
        barX = point.plotX + pointXOffset,
            ...
            bottom,
            fromTop,
            barH = mathMax(plotY, yBottom) - barY; // Handle options.minPointLength
        ...
        point.barX = barX;
        point.pointWidth = pointWidth; // Fix the tooltip on center of grouped columns (#1216, #424)

        point.tooltipPos = chart.inverted ? [yAxis.len - plotY, series.xAxis.len - barX - barW / 2] : [barX + barW / 2, plotY + yAxis.pos - chart.plotTop]; // Round off to obtain crisp edges and avoid overlapping with neighbours (#2694)
        ...
        barH = bottom - barY; // Top edges are exceptions
      ...
      each(series.points, function (point) {
        ...
        if (plotY !== UNDEFINED && !isNaN(plotY) && point.y !== null) {
          ...
          if (graphic) {
            ...
            point.graphic = graphic = renderer[point.shapeType](shapeArgs).attr(pointAttr).attr(borderAttr).add(series.group).shadow(options.shadow, null, options.stacking && !options.borderRadius);
          }
        } else if (graphic) {
          point.graphic = graphic.destroy(); // #1269
        }
    ...
    animate: function animate(init) {
      ...
      if (hasSVG) {
        ...
        if (init) {
          ...
          series.group.animate(attr, series.options.animation); // delete this function to allow it only once
  ...
  var PiePoint = extendClass(Point, {
    ...
    init: function init() {
      ...
      var point = this,
          toggleSlice; // Disallow negative values (#1530)
      ...
      toggleSlice = function toggleSlice(e) {
    ...
    setVisible: function setVisible(vis) {
      var point = this,
          series = point.series,
          chart = series.chart; // if called without an argument, toggle visibility
     ...
     */
    slice: function slice(sliced, redraw, animation) {
      var point = this,
          series = point.series,
          chart = series.chart,
          translation;
      setAnimation(animation, chart); // redraw is true by default
  ...
  var PieSeries = {
    ...
    generatePoints: function generatePoints() {
      ...
      Series.prototype.generatePoints.call(this); // Populate local vars

      points = this.points;
      len = points.length; // Get the total sum
     ...
     */
    translate: function translate(positions) {
      this.generatePoints();
      var series = this,
          cumulative = 0,
          precision = 1000,
      ...
      radiusY,
          ...
          i,
          len = points.length,
          point; // Get positions - either an integer or a percentage string must be given.
      ...
      for (i = 0; i < len; i++) {
        point = points[i]; // set start and end angle
        ...
        point.angle = angle; // set the anchor point for data labels
    ...
    drawPoints: function drawPoints() {
      ...
      each(series.points, function (point) {
        graphic = point.graphic;
        shapeArgs = point.shapeArgs;
        shadowGroup = point.shadowGroup; // put the shadow behind all points
        ...
        if (graphic) {
          ...
          point.graphic = graphic = renderer[point.shapeType](shapeArgs).setRadialReference(series.center).attr(point.pointAttr[point.selected ? SELECT_STATE : NORMAL_STATE]).attr({
            'stroke-linejoin': 'round' //zIndex: 1 // #2722 (reversed)
     ...
     */
    sortByAngle: function sortByAngle(points, sign) {
    ...
    if (options.enabled || series._hasPointLabels) {
      ...
      each(points, function (point) {
        var enabled,
            ...
            isNew = true; // Determine if each data label is enabled
        ...
        if (dataLabel && !enabled) {
          ...
          rotation = options.rotation; // Get the string

          labelConfig = point.getLabelConfig();
          str = options.format ? format(options.format, labelConfig) : options.formatter.call(labelConfig, options); // Determine the color

          options.style.color = pick(options.color, options.style.color, series.color, 'black'); // update existing label
  ...
  Series.prototype.alignDataLabel = function (point, dataLabel, options, alignTo, isNew) {
    ...
    if (visible) {
      ...
      if (options.rotation) {
        ...
        alignAttr = dataLabel.alignAttr; // Handle justify or crop
  ...
  if (seriesTypes.pie) {
      ...
      while (i--) {
        ...
        if (distanceOption > 0) {
          ...
          for (j = 0; j < length; j++) {
            ...
            var closest = 9999,
                distance,
                slotI; // find the closest slot index
        ...
        for (j = 0; j < length; j++) {
          ...
          if (distanceOption > 0) {
            slot = usedSlots.pop();
            slotIndex = slot.i; // if the slot next to currrent slot is free, the y value is allowed
          ...
          dataLabel.connX = x;
          dataLabel.connY = y; // Detect overflowing data labels

          if (this.options.size === null) {
            dataLabelWidth = dataLabel.width; // Overflow left
            ...
            if (dataLabel && dataLabel._pos) {
              ...
              if (connector) {
                ...
                point.connector = connector = series.chart.renderer.path(connectorPath).attr({
                  ...
                  visibility: visibility //zIndex: 0 // #2722 (reversed)
    ...
    seriesTypes.pie.prototype.verifyDataLabelOverflow = function (overflow) {
      var center = this.center,
          ...
          newSize = minSize,
          ret; // Handle horizontal size and center
  ...
  if (seriesTypes.column) {
    ...
    seriesTypes.column.prototype.alignDataLabel = function (point, dataLabel, options, alignTo, isNew) {
      ...
      options.verticalAlign = pick(options.verticalAlign, inverted || inside ? 'middle' : below ? 'top' : 'bottom'); // Call the parent method
    ...
    drawTrackerGraph: function drawTrackerGraph() {
      ...
      if (tracker) {
        ...
        series.tracker = renderer.path(trackerPath).attr({
          ...
          zIndex: 2
        }).add(series.group); // The tracker is added to the series group, which is clipped, but is covered
  ...
  extend(Legend.prototype, {
    setItemEvents: function setItemEvents(item, legendItem, useHTML, itemStyle, itemHiddenStyle) {
      var legend = this; // Set the events on the item group, or in case of useHTML, the item itself (#1249)
  ...
  extend(Point.prototype, {
    ...
    select: function select(selected, accumulate) {
      var point = this,
          series = point.series,
          chart = series.chart;
      selected = pick(selected, !point.selected); // fire the event with the defalut handler
      ...
      point.firePointEvent(selected ? 'select' : 'unselect', {
        ...
        point.setState(selected && SELECT_STATE); // unselect all other points unless Ctrl or Cmd + click
    ...
    onMouseOver: function onMouseOver(e) {
      var point = this,
          ...
          chart = series.chart,
          tooltip = chart.tooltip,
          hoverPoint = chart.hoverPoint; // set normal state to previous series
    ...
    setState: function setState(state, move) {
      ...
      if (point.graphic) {
        ...
        if (state && markerStateOptions) {
          ...
          newSymbol = pointMarker.symbol || series.symbol; // If the point has another symbol than the previous one, throw away the
  ...
  extend(Series.prototype, {
    ...
    onMouseOut: function onMouseOut() {
      ...
      var series = this,
          ...
          chart = series.chart,
          tooltip = chart.tooltip,
          hoverPoint = chart.hoverPoint; // trigger mouse out on the point, which must be in this series
     ...
     */
    setTooltipPoints: function setTooltipPoints(renew) {
      var series = this,
          points = [],
          pointsLength,
          low,
      ...
      for (i = 0; i < pointsLength; i++) {
        ...
        if (pointX >= xExtremes.min && pointX <= xExtremes.max) {
          // #1149
          nextPoint = points[i + 1]; // Set this range's low to the last range's high plus one

          low = high === UNDEFINED ? 0 : high + 1; // Now find the new high
    ...
    select: function select(selected) {
      var series = this; // if called without an argument, toggle
